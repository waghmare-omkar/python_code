...
class AppGUI:
    def __init__(self, window_title, widthXheight="700x540"):
        ...
        self.frame1.pack()
        self.canvas = tk.Canvas(self.frame1, width=self.video.width,
        ...
        #    file="Vlog_Master_Files\\record_icon.ico")
        self.button_rec = tk.Button(self.frame2, text="Record Vlog",
                                    compound="left")
        ...
        self.label_bottom = tk.Label(
            self.app_window, text="Press the record button to start vlogging")
        self.label_bottom.place(relx=1.0, rely=1.0, anchor='se')

        # Creating one more button for face detection toggle
        # rec_photo2 = PIL.ImageTk.PhotoImage(
        #    file="Vlog_Master_Files\\face_icon.ico")
        self.button_face_detect = tk.Button(
            self.frame2, text="Detect Faces",
            compound="left")
        self.button_face_detect.state = False
        self.button_face_detect.bind(
            '<ButtonRelease-1>', self.toggle_face_detection)
    ...
    def update(self):
        ...
        if ret:
            if self.button_face_detect.state is True:
                ...
                faces = self.video.face_cascade.detectMultiScale(
                    cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY), 1.3, 5)
            ...
            frame = cv2.putText(frame, text_date, org_date, font, fontScale,
                                color, thickness, cv2.LINE_AA, False)
            frame = cv2.putText(frame, text_time, org_time, font, fontScale,
                                color, thickness, cv2.LINE_AA, False)
